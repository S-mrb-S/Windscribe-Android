...
//        }
    ...
    fun update(): Completable {
        ...
        Log.d("MRBT", "Step 1 update")
                    logger.debug("Parsing server list JSON")
                    Log.d("MRBT", "Step 1 Parse!")
                        val dataArray = """
                            [
                              {
                                "id": 7,
    ...
    fun updateNoCompletable(){
        ...
        val regions:List<Region> = Gson().fromJson<List<Region>>(
            dataArray,
            object : TypeToken<ArrayList<Region?>?>() {}.type
        )

        addToStaticData(regions)

    }

    private fun addToStaticData(regions: List<Region>) {
        ...
        try{
            ...
            Log.d("MRBT", "Step 1 database added")
            sp.windscribe.vpn.qq.Data.regsion = regions
            sp.windscribe.vpn.qq.Data.cities = cities
        }catch (e: Exception){
            Log.d("MRBT", "Step 1 database not added")
            Log.d("MRBT", e.toString())
        }finally {
